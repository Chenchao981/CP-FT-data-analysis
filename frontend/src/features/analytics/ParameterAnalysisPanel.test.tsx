...
const analysisResult: DatasetParameterAnalysisResult = {
  ...
  capabilities: [
    ...
    { code: "CAPABILITY", status: "GATED", reason_code: "CAPABILITY_RULE_REQUIRED" },
  ],
  counts: {
    input_units: 10,
    included_units: 4,
    ...
    parameters: [{
      ...
      box_plot: {
        ...
        histogram: {
          ...
          bins: [
          { index: 0, lower_bound: 0, upper_bound: 5, count: 3, lower_inclusive: true, upper_inclusive: false },
          { index: 1, lower_bound: 5, upper_bound: 10, count: 1, lower_inclusive: true, upper_inclusive: true },
        ],
      },
      capability: {
        status: "NOT_ELIGIBLE",
...
describe("ParameterAnalysisPanel", () => {
  ...
  it("runs only after an explicit click and preserves the backend chart/statistics contract", async () => {
    ...
    expect(screen.getByText(/BoxPlot、Histogram 和显式 Capability Rule/)).toBeInTheDocument();
    await selectValue("参数分析参数", "VTH");
    await selectValue("参数分析总体结果", "PASS");
    await selectValue("参数分析总体结果", "UNKNOWN");
    ...
    const submitted = vi.mocked(analyzeDatasetParameters).mock.calls[0][0];
    expect(submitted).not.toHaveProperty("capability.rule_code");
    ...
    expect(screen.getByText("PROGRAM_LIMIT")).toBeInTheDocument();
    ...
    const boxRow = screen.getByText("TUKEY_1_5_IQR").closest("tr");
    ...
    expect(histogramOption.series[0].data).toEqual([3, 1]);
    expect(screen.getByText(/方法 EQUAL_WIDTH_FIXED_BINS_LAST_CLOSED_V1/)).toBeInTheDocument();
    expect(screen.getByText("统计规则尚未批准，能力指数保持关闭")).toBeInTheDocument();
    expect(screen.getAllByText("CAPABILITY_RULE_REQUIRED").length).toBeGreaterThan(0);
    expect(screen.getByText("后端未选择 Cpk 子组规则")).toBeInTheDocument();
  }, 20_000);

  it("fails closed when a detail-dataset source filter would be applied across datasets", async () => {
    renderPanel({ ...panelProps, sourceIds: ["SOURCE-1"] });
    await selectValue("参数分析参数", "VTH");

    expect(screen.getByText("跨 Dataset 参数分析不能沿用当前源文件筛选")).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "执行参数分析" })).toBeDisabled();
    expect(analyzeDatasetParameters).not.toHaveBeenCalled();
  });
