...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => { const expired = () => reset(); window.addEventListener("tms-auth-expired", expired); getMe().then(setUser).catch(reset).finally(() => setLoading(false)); return () => window.removeEventListener("tms-auth-expired", expired); }, [reset]);
