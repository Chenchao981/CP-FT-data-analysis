...
import { listCpResults, listCpUploads, ProductionResultRow, ProductionUploadRow, uploadCpData } from "../../api/productionData";
...
export function CpDataWorkbench() {
  ...
  const uploads = useQuery({ queryKey: ["production", "cp", "uploads"], queryFn: listCpUploads });
  const results = useQuery({ queryKey: ["production", "cp", "results"], queryFn: listCpResults });
  ...
  const mutation = useMutation({
    mutationFn: async (values: { factory_code: string; remark?: string }) => {
      ...
      if (!nativeFiles.length) throw new Error("请选择CP源文件");
      return uploadCpData(nativeFiles, values.factory_code, values.remark);
    },
    onSuccess: async (data) => { messageApi.success(`上传和清洗完成，批次 ${data.import_batch_id}`); setOpen(false); setFiles([]); form.resetFields(); await queryClient.invalidateQueries({ queryKey: ["production", "cp"] }); },
    onError: (error) => messageApi.error(error.message),
  });
  const uploadColumns: ColumnsType<ProductionUploadRow> = [
    ...
    { title: "晶圆厂", dataIndex: "factory_code", width: 100, render: () => "华虹" },
  ...
  const resultColumns: ColumnsType<ProductionResultRow> = [
    ...
    { title: "晶圆数", dataIndex: "wafer_count", width: 90 },
    { title: "晶圆厂", dataIndex: "factory_code", width: 100, render: () => "华虹" },
  ...
  return <div className="workbench production-workbench">
    ...
    {(uploads.isError || results.isError) && <Alert type="error" showIcon message="CP数据加载失败" description={(uploads.error ?? results.error)?.message} />}
    ...
    <Modal title="上传量产CP数据" open={open} width={700} onCancel={() => !mutation.isPending && setOpen(false)} onOk={() => form.submit()} okText="上传并自动清洗" confirmLoading={mutation.isPending} destroyOnHidden>
      ...
      <Form form={form} layout="vertical" initialValues={{ factory_code: "huahong" }} onFinish={(values) => mutation.mutate(values)} className="cp-upload-form">
        <Form.Item label="业务分类"><Space><Tag color="blue">量产</Tag><Tag color="cyan">CP数据</Tag></Space></Form.Item>
        <Form.Item label="晶圆厂" name="factory_code" rules={[{ required: true }]}><Select options={[{ value: "huahong", label: "华虹" }]} /></Form.Item>
        <Form.Item label="CP源文件" required><Upload.Dragger multiple accept=".zip,.7z,.txt" fileList={files} beforeUpload={() => false} onChange={({ fileList }) => setFiles(fileList)}><p className="ant-upload-drag-icon"><FileSearchOutlined /></p><p className="ant-upload-text">点击或拖入CP源文件</p><p className="ant-upload-hint">支持华虹 ZIP、7Z 或多个 TXT；上传后自动复用现有清洗逻辑。</p></Upload.Dragger></Form.Item>
