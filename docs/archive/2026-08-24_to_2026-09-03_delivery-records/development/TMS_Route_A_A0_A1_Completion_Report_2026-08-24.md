...
> 2026-08-24 业务确认：以下内容按“先完成功能、让程序跑起来”重新排序。CP/FT 程序统一、多批次 Spec 新规则、数据库安全讨论和前端拆包均不作为当前开发阻塞项。
...
业务端已要求用户只有在 Spec 相同时才把批次纳入同一次比较。首版不再设计多 Lot Spec fingerprint、自动合并或逐 Lot Binding，直接沿用现有程序取第一批次 Spec 的规则。该项不再阻塞 A2。
...
数据库位于安全的隔离内网，数据库安全不再作为本阶段讨论或阻塞项。前端生产构建已经通过，JavaScript Chunk 体积优化延后到核心功能完成之后，也不再列为当前问题。

### 8. CP/FT 与四个上传入口

CP Cleaner 和 FT Cleaner 维持两个独立程序。前端维持工程-CP、工程-FT、量产-CP、量产-FT 四个上传入口；用户从哪个入口上传，系统就按该入口确定业务域和 CP/FT 类型，不增加统一入口或自动数据类型识别。
...
5. 多批次分析沿用第一批次 Spec，并在界面提示只选择相同 Spec 批次。
