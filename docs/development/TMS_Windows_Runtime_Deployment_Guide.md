...
本指南部署 API、Route A Worker、Analytics Export Worker、FTP Collection Worker 与三个独立 Cleanup 角色。
...
| `TMS-FtpCollectionWorker` | FTP/FTPS 只读采集与正式入库调度 | Windows开机 | 每1分钟重启，最多20次 |
...
七个任务均使用`IgnoreNew`并发策略：前一实例仍在运行时，不启动第二实例。API 与三个 Worker 无执行时限；三个 Cleanup 单次最多2小时。三个 Cleanup 使用不同受管根、不同计划任务和独立删除审批，不能合并。
...
- 当前唯一 Schema head 为 `sql2014_0029`；部署脚本动态核对发布包 head，不能硬编码或绕过不一致。
- FTP 凭据在 API/采集 Worker 实际 Windows 账号下通过 `set_tms_ftp_credential.ps1 -Reference <引用> -PythonPath <包外Python>` 交互录入。SQL 只保存引用，不保存密码。详见 [FTP 设计](../architecture/NCE_PYMS_FTP_Integration_Design_2026-09-05.md)。
...
5. 生产数据库已完成 pre-check、`COPY_ONLY,CHECKSUM` 备份和 `RESTORE VERIFYONLY`，并迁移至 `sql2014_0029`；正式变更前还必须在独立空测试库执行 0001→0029。其中 0028 为全量物理分表迁移，须停止写入并核对 SQL Server 自身的数据盘与日志盘容量；0029 仅新增 FTP 采集表。有采集配置或记录时禁止自动回退 0029。具体分表要求见 [物理存储合同](../architecture/NCE_PYMS_Physical_Stage_Storage_2026-09-05.md)。
...
- `api.jsonl`
- `worker.jsonl`
- `analytics-export-worker.jsonl`
- `ftp-collection-worker.jsonl`
- `cleanup.jsonl`
...
卸载只停止并移除七个计划任务，不删除数据库数据、Workspace、日志、运行环境或项目文件。
...
5. 在独立空测试库验证 0001→0029，再对目标库 Migration 和 post-check；
6. 注册七个计划任务，核对 API ready、各 Worker ready file 和正式清洗 Worker registry 的 Database/Server/Schema/Worker ID；
