...
CREATE TABLE ingestion.import_batch_file (
    ...
    CONSTRAINT CK_import_batch_file_detection_json CHECK(detection_evidence_json IS NULL OR ISJSON(detection_evidence_json)=1)
);
GO
...
CREATE TABLE ingestion.format_profile (
    ...
    CONSTRAINT CK_format_profile_signature_json CHECK(ISJSON(signature_json)=1),
    CONSTRAINT CK_format_profile_role_json CHECK(ISJSON(file_role_contract_json)=1)
);
GO
...
CREATE TABLE ingestion.dq_rule_version (
    ...
    CONSTRAINT CK_dq_rule_version_json CHECK(parameters_json IS NULL OR ISJSON(parameters_json)=1)
);
GO
...
CREATE TABLE dataset.dataset_version (
    ...
    CONSTRAINT CK_dataset_version_json CHECK(metadata_json IS NULL OR ISJSON(metadata_json)=1)
);
GO
...
CREATE TABLE evaluation.rule_version (
    ...
    CONSTRAINT CK_evaluation_rule_parameters_json CHECK(ISJSON(parameters_json)=1)
);
GO
...
CREATE TABLE evaluation.evaluation_run (
    ...
    CONSTRAINT CK_evaluation_run_filter_json CHECK(ISJSON(filter_json)=1)
);
GO
...
CREATE TABLE evaluation.metric_result (
    ...
    CONSTRAINT CK_metric_result_json CHECK(ISJSON(result_json)=1)
);
GO
...
CREATE TABLE analysis.saved_analysis (
    ...
    CONSTRAINT CK_saved_analysis_filter_json CHECK(ISJSON(filter_json)=1),
    CONSTRAINT CK_saved_analysis_chart_json CHECK(ISJSON(chart_config_json)=1),
    CONSTRAINT CK_saved_analysis_eval_json CHECK(evaluation_context_json IS NULL OR ISJSON(evaluation_context_json)=1)
);
GO
