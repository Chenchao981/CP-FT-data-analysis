...
CREATE TABLE mdm.supplier (
    ...
    CONSTRAINT CK_supplier_json CHECK(metadata_json IS NULL OR ISJSON(metadata_json)=1)
);
GO
...
CREATE TABLE mdm.product (
    ...
    CONSTRAINT CK_product_json CHECK(metadata_json IS NULL OR ISJSON(metadata_json)=1)
);
GO
...
CREATE TABLE mdm.test_program_version (
    ...
    CONSTRAINT CK_program_version_json CHECK(metadata_json IS NULL OR ISJSON(metadata_json)=1)
);
GO
...
CREATE TABLE mdm.test_item_definition (
    ...
    CONSTRAINT CK_test_item_condition_json CHECK(condition_json IS NULL OR ISJSON(condition_json)=1)
);
GO
...
CREATE TABLE mdm.spec_set (
    ...
    CONSTRAINT CK_spec_set_json CHECK(metadata_json IS NULL OR ISJSON(metadata_json)=1)
);
GO
...
CREATE TABLE mdm.spec_item (
    ...
    CONSTRAINT CK_spec_item_json CHECK(condition_json IS NULL OR ISJSON(condition_json)=1)
);
GO
...
CREATE TABLE mdm.bin_definition (
    ...
    CONSTRAINT CK_bin_definition_json CHECK(metadata_json IS NULL OR ISJSON(metadata_json)=1)
);
GO
...
CREATE TABLE ingestion.import_batch (
    ...
    CONSTRAINT CK_import_batch_json CHECK(metadata_json IS NULL OR ISJSON(metadata_json)=1)
);
GO
...
CREATE TABLE ingestion.source_file (
    ...
    CONSTRAINT CK_source_file_json CHECK(metadata_json IS NULL OR ISJSON(metadata_json)=1)
);
GO
...
CREATE TABLE ingestion.source_file_receipt (
    ...
    CONSTRAINT CK_receipt_json CHECK(metadata_json IS NULL OR ISJSON(metadata_json)=1)
);
GO
...
CREATE TABLE ingestion.parser_profile (
    ...
    CONSTRAINT CK_parser_json CHECK(detect_rules_json IS NULL OR ISJSON(detect_rules_json)=1)
);
GO
...
CREATE TABLE ingestion.processing_job (
    ...
    CONSTRAINT CK_job_json CHECK(metadata_json IS NULL OR ISJSON(metadata_json)=1)
);
GO
...
CREATE TABLE ingestion.processing_run (
    ...
    CONSTRAINT CK_processing_run_json CHECK(metadata_json IS NULL OR ISJSON(metadata_json)=1)
);
GO
...
CREATE TABLE test.test_run (
    ...
    CONSTRAINT CK_test_run_json CHECK(metadata_json IS NULL OR ISJSON(metadata_json)=1)
);
GO
...
CREATE TABLE test.unit_result (
    ...
    CONSTRAINT CK_unit_json CHECK(metadata_json IS NULL OR ISJSON(metadata_json)=1)
);
GO
...
GO
CREATE CLUSTERED COLUMNSTORE INDEX CCI_measurement ON test.measurement;
GO
...
CREATE TABLE trace.unit_traceability (
    ...
    CONSTRAINT CK_trace_json CHECK(metadata_json IS NULL OR ISJSON(metadata_json)=1)
);
GO
