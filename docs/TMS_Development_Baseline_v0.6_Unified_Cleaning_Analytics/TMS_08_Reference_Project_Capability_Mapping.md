...
1. 先登记 Format Profile 和黄金样例，不从旧代码数量推断“已支持”。
2. 每个格式独立验收文件数、行数、Lot/批次、Wafer/Unit、Bin、参数、单位、规格和代表值。
3. 兼容导出与内部 Canonical Model 分开维护。
4. 图表必须读取明确 Dataset Version；不得扫描“最新文件”。
5. 未迁移格式继续由旧系统生产使用，直至新平台黄金样例对账通过。
