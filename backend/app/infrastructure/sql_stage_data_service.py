...
class SqlStageDataService:
    ...
    def register_upload(
        ...
        with self._engine.begin() as connection:
            ...
            ), {"login": principal.login_name, "owner": principal.user_id, "domain": business_domain, "stage": stage_label, "factory": factory_code, "name": batch_name, "remark": remark, "metadata": json.dumps({"uploader_user_id": principal.user_id}, ensure_ascii=False)}).scalar_one())
            for ordinal, item in enumerate(files, start=1):
                ...
                ), {"batch": batch_id, "receipt": receipt_id, "ordinal": ordinal, "evidence": json.dumps({"business_domain": business_domain, "test_stage": stage_label, "factory": factory_code}, ensure_ascii=False)})
        return batch_id
    ...
    def record_result(self, batch_id: int, job_id: int, result: dict) -> None:
        with self._engine.begin() as connection:
            ...
            ), {"batch": batch_id, "job": job_id, "name": result["data_name"], "product": result.get("product_name"), "lot": result.get("lot_id"), "wafers": result.get("wafer_count"), "factory": result["factory_code"], "output": result["output_uri"], "items": result.get("test_item_count"), "units": result.get("unit_count"), "passes": result.get("pass_count"), "yield_rate": result.get("yield_rate"), "data_type": result.get("data_type", "CP"), "manifest": json.dumps(result.get("artifacts", []), ensure_ascii=False)})
