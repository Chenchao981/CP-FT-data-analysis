...
$context = Get-TmsRuntimeContext -Role 'ftp-collection-worker'
$entryPoint = Join-Path $context.Workspace 'scripts\run_ftp_collection_worker.py'
if ($ValidateOnly) {
...
$resolvedWorkerId = if ($PSBoundParameters.ContainsKey('WorkerId')) {
    ...
    [Environment]::GetEnvironmentVariable('TMS_FTP_WORKER_ID')
}
...
foreach ($option in @(
    ...
    @('ReadyFile', '--ready-file', 'TMS_FTP_WORKER_READY_FILE'),
    @('StopFile', '--stop-file', 'TMS_FTP_WORKER_STOP_FILE')
)) {
