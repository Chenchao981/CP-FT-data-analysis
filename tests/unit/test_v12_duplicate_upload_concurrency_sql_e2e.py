...
def test_v12_concurrency_database_guard_is_exact() -> None:
    ...
    with pytest.raises(RuntimeError, match="restricted to TMS_G0_DEV/sql2014_0029"):
        verifier._assert_database_identity(
            {"database": "TMS_PROD", "schema_revision": "sql2014_0029"}
        )
    with pytest.raises(RuntimeError, match="restricted to TMS_G0_DEV/sql2014_0029"):
        verifier._assert_database_identity(
...
def test_v12_concurrency_e2e_has_exact_cleanup_and_no_filesystem_writes() -> None:
    ...
    assert 'EXPECTED_SCHEMA_REVISION = "sql2014_0029"' in source
