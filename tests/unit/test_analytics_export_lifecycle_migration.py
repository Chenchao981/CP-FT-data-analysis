...
def test_analytics_export_runners_and_global_verifier_require_current_head() -> None:
    ...
    assert 'schema_revision"] != "sql2014_0029"' in worker
    assert 'schema_revision"] != "sql2014_0029"' in cleanup
    assert 'assert revision == "sql2014_0029"' in verifier
...
def test_real_lifecycle_smoke_write_guard_requires_exact_dev_target() -> None:
    assert_lifecycle_write_target(
        {"database": "TMS_G0_DEV", "schema_revision": "sql2014_0029"}
    )
    with pytest.raises(RuntimeError, match="TMS_G0_DEV"):
        assert_lifecycle_write_target(
            {"database": "NCE_TMS", "schema_revision": "sql2014_0029"}
        )
    with pytest.raises(RuntimeError, match="sql2014_0029"):
        assert_lifecycle_write_target(
