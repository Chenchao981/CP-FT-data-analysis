...
def test_v12_sql_e2e_database_guard_is_exact() -> None:
    ...
    with pytest.raises(RuntimeError, match="restricted to TMS_G0_DEV/sql2014_0028"):
        verifier._assert_database_identity(
            {"database": "TMS_PROD", "schema_revision": "sql2014_0028"}
        )
    with pytest.raises(RuntimeError, match="restricted to TMS_G0_DEV/sql2014_0028"):
        verifier._assert_database_identity(
...
def test_v12_sql_e2e_is_rollback_only_and_uses_application_services() -> None:
    ...
    assert 'EXPECTED_SCHEMA_REVISION = "sql2014_0028"' in source
